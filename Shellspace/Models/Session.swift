...
extension String {
    ...
    /// Claude CLI resolves symlinks internally, so Shellspace must do the same
...
final class Session {
    ...
    static var centralLogsDir: URL {
        let path = NSString("~/Library/CloudStorage/Dropbox/Buzzbox/Shellspace/logs").expandingTildeInPath
    ...
    var legacyLogPath: URL {
        let logsDir = URL(fileURLWithPath: projectPath).appendingPathComponent(".shellspace-logs")
...
extension UTType {
    static let session = UTType(exportedAs: "com.buzzbox.shellspace.session")
}
