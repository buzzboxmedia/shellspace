...
private let syncLogger = Logger(subsystem: "com.buzzbox.shellspace", category: "SessionSync")
...
class SessionSyncService {
    ...
    /// Check if Dropbox is available on this machine
    static var isDropboxAvailable: Bool {
        ...
        return FileManager.default.fileExists(atPath: newPath) || FileManager.default.fileExists(atPath: legacyPath)
    }

    /// Base Dropbox path (prefers new CloudStorage location)
    private static var dropboxBasePath: String? {
        let newPath = NSString("~/Library/CloudStorage/Dropbox").expandingTildeInPath
        let legacyPath = NSString("~/Dropbox").expandingTildeInPath
    ...
    static var centralSessionsDir: URL? {
        guard let base = dropboxBasePath else { return nil }
        return URL(fileURLWithPath: "\(base)/Buzzbox/Shellspace/sessions")
    }

    /// Centralized projects directory in Dropbox
    static var centralProjectsDir: URL? {
        guard let base = dropboxBasePath else { return nil }
        return URL(fileURLWithPath: "\(base)/Buzzbox/Shellspace/projects")
    }

    /// Whether sync is actually available (enabled AND Dropbox exists)
    var isSyncAvailable: Bool {
        isEnabled && Self.isDropboxAvailable
    }
    ...
    func exportProject(_ project: Project) {
        guard isSyncAvailable, let projectsDir = Self.centralProjectsDir else { return }
    ...
    func exportSession(_ session: Session) {
        guard isSyncAvailable, let sessionsDir = Self.centralSessionsDir else { return }
    ...
    func importAllSessions(modelContext: ModelContext) {
        guard isSyncAvailable else {
            if !Self.isDropboxAvailable {
                syncLogger.debug("Dropbox not available, skipping sync")
            } else {
    ...
    private func importAllProjects(modelContext: ModelContext) {
        guard let projectsDir = Self.centralProjectsDir else {
            syncLogger.debug("No Dropbox projects directory available")
            return
        }
    ...
    private func importSessions(modelContext: ModelContext) {
        guard let sessionsDir = Self.centralSessionsDir else {
            syncLogger.debug("No Dropbox sessions directory available")
            return
        }
