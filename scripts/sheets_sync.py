#!/usr/bin/env python3
"""
Google Sheets billing sync for ClaudeHub.
Pushes completed task billing entries to the spreadsheet.

Usage:
    python3 sheets_sync.py log --workspace "INFAB" --task "Fix checkout" --description "Fixed bug" --est-hours 2 --actual-hours 1.5
...
# Spreadsheet config
DEFAULT_SPREADSHEET_NAME = "Buzzbox Billing Log"
SHEET_NAME = "Billing"

# Workspaces with their own spreadsheets
WORKSPACE_SPREADSHEETS = {
    "Talkspresso": "Talkspresso Billing Log",
    "Miller": "Miller Billing Log",
}

# Column headers for billing
HEADERS = ["Date", "Client", "Project", "Task", "Description", "Est Hrs", "Actual Hrs", "Status"]
...
def get_spreadsheet_id_file(workspace: str = None):
    ...
    if workspace and workspace in WORKSPACE_SPREADSHEETS:
        return CONFIG_DIR / f"billing_sheet_id_{workspace.lower()}.txt"
    return CONFIG_DIR / "billing_sheet_id.txt"
...
def create_spreadsheet(workspace: str = None):
    """Create a new spreadsheet with headers and styling."""
        ...
        body={
            "requests": [
                ...
                {"updateDimensionProperties": {
                    ...
                    "properties": {"pixelSize": 100}, "fields": "pixelSize"  # Client
                }},
                {"updateDimensionProperties": {
                ...
                {"updateDimensionProperties": {
                    ...
                    "properties": {"pixelSize": 180}, "fields": "pixelSize"  # Task
                }},
                {"updateDimensionProperties": {
                    "range": {"sheetId": sheet_id, "dimension": "COLUMNS", "startIndex": 4, "endIndex": 5},
                    "properties": {"pixelSize": 350}, "fields": "pixelSize"  # Description
                }},
                {"updateDimensionProperties": {
                ...
                {"setBasicFilter": {
                    "filter": {
                        "range": {"sheetId": sheet_id, "startRowIndex": 0, "startColumnIndex": 0, "endColumnIndex": 8}
                    }
                }},
            ]
        }
...
def log_billing(workspace: str, project: str, task: str, description: str,
                est_hours: float, actual_hours: float, status: str = "billed"):
    """Log a billing entry to the spreadsheet."""
...
def main():
    parser = argparse.ArgumentParser(description="ClaudeHub Billing Sync")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # Log billing command
    log_parser = subparsers.add_parser("log", help="Log a billing entry")
    log_parser.add_argument("--workspace", required=True, help="Client/workspace name")
    ...
    log_parser.add_argument("--status", default="billed", help="Billing status")

    # Init command
    subparsers.add_parser("init", help="Initialize billing spreadsheet")
